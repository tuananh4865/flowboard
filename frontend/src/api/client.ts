...
export async function upscaleMedia(mediaId: string, resolution: string): Promise<UpscaleResponse> {
  const clean = mediaId.replace(/^media\//, "");
  ...
  return api<UpscaleResponse>(`/api/media/${encodeURIComponent(clean)}/upscale`, {
    method: "POST",
